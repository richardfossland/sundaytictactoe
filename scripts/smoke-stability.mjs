...
console.log("SundayTicTacToe LIVE stability smoke\n");

// ---- 1. ten truly concurrent joins (same NAT IP) all succeed ----
{
  const t = await api("POST", "/api/tournament", { title: "StressJoin" });
  const names = Array.from({ length: 10 }, (_, i) => `Spiller${i + 1}`);
  const results = await Promise.all(
    names.map((n) => api("POST", "/api/join", { pin: t.joinPin, displayName: n })),
  );
...
{
  const t = await api("POST", "/api/tournament", { title: "DoubleStart" });
  for (const n of ["A", "B", "C", "D"]) await api("POST", "/api/join", { pin: t.joinPin, displayName: n });
...
{
  const t = await api("POST", "/api/tournament", { title: "ExtendTwice", config: { roundTimerSec: 300 } });
  for (const n of ["A", "B"]) await api("POST", "/api/join", { pin: t.joinPin, displayName: n });
  await api("POST", "/api/round/start", { tournamentId: t.id, hostCode: t.hostCode });
  const e1 = await api("POST", "/api/round/extend", { tournamentId: t.id, hostCode: t.hostCode });
  const e2 = await api("POST", "/api/round/extend", { tournamentId: t.id, hostCode: t.hostCode });
  const b = await board(t.id);
  const round = b.rounds.find((r) => r.number === 1);
  if (e1.extendedMs === null) {
    console.log("  ⚠ extend running in pre-0007 fallback (started_at shift) — apply 0007 to enable atomic extensions");
...
{
  const t = await api("POST", "/api/tournament", { title: "OrderCup", config: { format: "cup" } });
  const names = ["A", "B", "C", "D", "E", "F", "G", "H"];
  for (const n of names) await api("POST", "/api/join", { pin: t.joinPin, displayName: n });
  await api("POST", "/api/round/start", { tournamentId: t.id, hostCode: t.hostCode });
  ...
  if (!slotsMigrated) {
    console.log("  ⚠ games.slot all 0 (0007 not migrated) — resolving in fetch order; re-run after applying 0007 for the full out-of-order assertion");
  }
  ...
  const adv = await api("POST", "/api/round/advance", { tournamentId: t.id, hostCode: t.hostCode });
