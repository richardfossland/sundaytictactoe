...
## Current backlog (`npm run port:status`, 2026-09-06 snapshot, post-fix)

An earlier snapshot of this section (see git history) found the script
reporting 20 items, most of them false positives: `port::extract_port_pr_refs`
only recognised a `Port of sundaychess#<NN>` **body** line, so a squash-merge
**title** like `"... (port of chess #92/#93/#94/#97) (#51)"` — with no
separate body line — didn't register, even though the PR had genuinely
already ported that chess work. That snapshot's own recommendation ("extend
`port::extract_port_pr_refs` to also recognise `(port of chess
#NN[/#NN...])` in a commit subject") is now implemented: the matcher
recognises both shapes, case-insensitively, on either the subject or the
body, anchored tightly enough that a trailing GitHub squash PR number (the
`(#51)` after the closing paren) is never swept in alongside the chess PR
numbers it actually names. `scripts/port-ignore.txt` also picked up entries
for the handful of commits neither convention could ever catch: chess
#37–#41 (pre-date TTT's own clone by a day, same class as the existing
#44–#46/#48–#50 entries), #100 (chess-only coach-lesson content — only
non-excluded file is a test for the excluded `lib/coach/lessons.ts` module),
and #106 (eslint 9→10, done independently as TTT #54 the same day, not a
literal port).
...
Both are genuinely open:

- **#90** — a `vitest.config.ts` testTimeout bump for engine specs; generic
  infra, not yet ported.
- **#105** (per-tournament session keys) — confirmed not ported:
  `lib/client/identity.ts` still keys everything off a single
  `"ttt:player"` slot per browser, the exact design #105 replaced in chess.
